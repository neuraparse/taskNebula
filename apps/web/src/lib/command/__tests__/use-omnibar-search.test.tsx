...
describe('useOmnibarSearch', () => {
  it('POSTs the hybrid endpoint with the body its schema expects and maps hrefs to /issues/[issueId]', async () => {
  ...
  it('falls back to GET /api/search on a non-ok hybrid response (e.g. 405)', async () => {
    ...
    const { result } = renderHook(() =>
      useOmnibarSearch({ query: 'fallback', tab: 'all', organizationId: 'org_1', debounceMs: 0 })
    );
  ...
  it('surfaces an error when both endpoints fail', async () => {
    ...
    const { result } = renderHook(() =>
      useOmnibarSearch({ query: 'boom', tab: 'all', organizationId: 'org_1', debounceMs: 0 })
    );
