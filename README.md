...
| **Pinned production**  | Repeatable self-hosted releases                     | `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.6 docker compose up -d`                                                                       |
...
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.6 \
...
| Recommended tag   | `0.6.6` for pinned installs, `latest` for quickstart demos                                                     |
...
| Immutable pulls   | Inspect the tag digest with `docker buildx imagetools inspect neuraparse/tasknebula:0.6.6` before pinning hard |
...
Latest release: **v0.6.6**. Highlights from the recent line — see the
...
```bash
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.6 docker compose up -d
```
...
| Area         | Recommendation                                                                  |
| ------------ | ------------------------------------------------------------------------------- |
| Image        | Pin `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.6`; use `latest` only for demos |
...
| Inspect image tag   | `docker buildx imagetools inspect neuraparse/tasknebula:0.6.6` |

> `docker-compose.yml` defaults to `image: neuraparse/tasknebula:latest`.
> Set `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.6` in `.env` to pin.
