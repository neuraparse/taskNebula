...
| **Pinned production**  | Repeatable self-hosted releases                     | `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.1 docker compose up -d`                                                                       |
...
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.1 \
...
| Recommended tag   | `0.6.1` for pinned installs, `latest` for quickstart demos                                                     |
...
| Immutable pulls   | Inspect the tag digest with `docker buildx imagetools inspect neuraparse/tasknebula:0.6.1` before pinning hard |
...
```bash
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.1 docker compose up -d
```
...
| Area         | Recommendation                                                                  |
| ------------ | ------------------------------------------------------------------------------- |
| Image        | Pin `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.1`; use `latest` only for demos |
...
| Inspect image tag   | `docker buildx imagetools inspect neuraparse/tasknebula:0.6.1` |

> `docker-compose.yml` defaults to `image: neuraparse/tasknebula:latest`.
> Set `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.6.1` in `.env` to pin.
