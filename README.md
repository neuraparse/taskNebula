...
| **Pinned production**  | Repeatable self-hosted releases                     | `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.3.4 docker compose up -d`                                                                       |
...
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.3.4 \
...
| Recommended tag   | `0.3.4` for pinned installs, `latest` for quickstart demos                                                     |
...
| Immutable pulls   | Inspect the tag digest with `docker buildx imagetools inspect neuraparse/tasknebula:0.3.4` before pinning hard |
...
```bash
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.3.4 docker compose up -d
```
...
| Area         | Recommendation                                                                  |
| ------------ | ------------------------------------------------------------------------------- |
| Image        | Pin `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.3.4`; use `latest` only for demos |
...
| Inspect image tag   | `docker buildx imagetools inspect neuraparse/tasknebula:0.3.4` |

> `docker-compose.yml` defaults to `image: neuraparse/tasknebula:latest`.
> Set `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.3.4` in `.env` to pin.
