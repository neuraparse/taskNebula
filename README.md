...
| **Pinned production**  | Repeatable self-hosted releases                     | `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.9 docker compose up -d`                                                                       |
...
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.9 \
...
| Recommended tag   | `0.7.9` for pinned installs, `latest` for quickstart demos                                                     |
...
| Immutable pulls   | Inspect the tag digest with `docker buildx imagetools inspect neuraparse/tasknebula:0.7.9` before pinning hard |
...
Latest release: **v0.7.9**. Highlights from the latest release line
(`v0.6.5` → `v0.7.9`) — see the [CHANGELOG](CHANGELOG.md) for the complete
history:
...
| Area         | Recommendation                                                                                  |
| ------------ | ----------------------------------------------------------------------------------------------- |
| Image        | Pin `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.9`; use `latest` only for demos                 |
...
| Inspect image tag   | `docker buildx imagetools inspect neuraparse/tasknebula:0.7.9` |

> `docker-compose.yml` defaults to `image: neuraparse/tasknebula:latest`.
> Set `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.9` in `.env` to pin.
