...
| **Pinned production**  | Repeatable self-hosted releases                     | `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.8 docker compose up -d`                                                                       |
...
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.8 \
...
| Recommended tag   | `0.7.8` for pinned installs, `latest` for quickstart demos                                                     |
...
| Immutable pulls   | Inspect the tag digest with `docker buildx imagetools inspect neuraparse/tasknebula:0.7.8` before pinning hard |
...
Latest release: **v0.7.8**. Highlights from the latest release line
(`v0.6.5` → `v0.7.8`) — see the [CHANGELOG](CHANGELOG.md) for the complete
history:

- **Appearance and profile hotfix.** Settings → Appearance now keeps explicit
  light/dark/system choices across navigation, and the lower-left account
  control is consistently circular with readable initials in light and dark
  mode.
...
```bash
TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.8 docker compose up -d
```
...
| Area         | Recommendation                                                                                  |
| ------------ | ----------------------------------------------------------------------------------------------- |
| Image        | Pin `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.8`; use `latest` only for demos                 |
...
| Inspect image tag   | `docker buildx imagetools inspect neuraparse/tasknebula:0.7.8` |

> `docker-compose.yml` defaults to `image: neuraparse/tasknebula:latest`.
> Set `TASKNEBULA_IMAGE=neuraparse/tasknebula:0.7.8` in `.env` to pin.
