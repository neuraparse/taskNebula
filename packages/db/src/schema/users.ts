...
export const users = pgTable(
  'users',
  {
    ...
    inviteTokenExpiresAt: timestamp('invite_token_expires_at', { mode: 'date' }),
    // Custom fields
...
export const accounts = pgTable(
  ...
  (table) => ({
    compoundKey: uniqueIndex('account_provider_account_idx').on(
      table.provider,
...
export const verificationTokens = pgTable(
  ...
  (table) => ({
    compoundKey: uniqueIndex('verification_token_identifier_token_idx').on(
      table.identifier,
      table.token
    ),
  })
